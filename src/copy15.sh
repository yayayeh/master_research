...
#L
	...
	if [[ -n "$(egrep 'Subject' $file_dir)" ]]; then #-n means ""is not empty
		...
		if [[ -z "$info" ]]; then #-z means "" is empty
			...
			#echo $(echo $info | awk '{print $2}')
			WL1n=$WL1n","$(echo $info | awk '{print $2}')
		fi
	fi

	info=$(egrep 'WL1p' $file_dir)
	if [[ -n "$(egrep 'Subject' $file_dir)" ]]; then #-n means ""is not empty
		WL1p=$WL1p", "
	else
		if [[ -z "$info" ]]; then #-z means "" is empty
			...
			#echo $(echo $info | awk '{print $2}')
			WL1p=$WL1p","$(echo $info | awk '{print $2}')
		fi
	fi

	info=$(egrep 'WL2n' $file_dir)
	if [[ -n "$(egrep 'Subject' $file_dir)" ]]; then #-n means ""is not empty
		WL2n=$WL2n", "
	else
		if [[ -z "$info" ]]; then #-z means "" is empty
			...
			#echo $(echo $info | awk '{print $2}')
			WL2n=$WL2n","$(echo $info | awk '{print $2}')
		fi
	fi

	info=$(egrep 'WL2p' $file_dir)
	if [[ -n "$(egrep 'Subject' $file_dir)" ]]; then #-n means ""is not empty
		WL2p=$WL2p", "
	else
		if [[ -z "$info" ]]; then #-z means "" is empty
			...
			#echo $(echo $info | awk '{print $2}')
			WL2p=$WL2p","$(echo $info | awk '{print $2}')
		fi
	fi
#M
	info=$(egrep 'WM1n' $file_dir)
	if [[ -n "$(egrep 'Subject' $file_dir)" ]]; then #-n means ""is not empty
		WM1n=$WM1n", "
	else
		if [[ -z "$info" ]]; then #-z means "" is empty
			...
			#echo $(echo $info | awk '{print $2}')
			WM1n=$WM1n","$(echo $info | awk '{print $2}')
		fi
	fi

	info=$(egrep 'WM1p' $file_dir)
	if [[ -n "$(egrep 'Subject' $file_dir)" ]]; then #-n means ""is not empty
		WM1p=$WM1p", "
	else
		if [[ -z "$info" ]]; then #-z means "" is empty
			...
			#echo $(echo $info | awk '{print $2}')
			WM1p=$WM1p","$(echo $info | awk '{print $2}')
		fi
	fi

	info=$(egrep 'WM2n' $file_dir)
	if [[ -n "$(egrep 'Subject' $file_dir)" ]]; then #-n means ""is not empty
		WM2n=$WM2n", "
	else
		if [[ -z "$info" ]]; then #-z means "" is empty
			...
			#echo $(echo $info | awk '{print $2}')
			WM2n=$WM2n","$(echo $info | awk '{print $2}')
		fi
	fi

	info=$(egrep 'WM2p' $file_dir)
	if [[ -n "$(egrep 'Subject' $file_dir)" ]]; then #-n means ""is not empty
		WM2p=$WM2p", "
	else
		if [[ -z "$info" ]]; then #-z means "" is empty
			...
			#echo $(echo $info | awk '{print $2}')
			WM2p=$WM2p","$(echo $info | awk '{print $2}')
		fi
	fi
#N
	info=$(egrep 'WN1n' $file_dir)
	if [[ -n "$(egrep 'Subject' $file_dir)" ]]; then #-n means ""is not empty
		WN1n=$WN1n", "
	else
		if [[ -z "$info" ]]; then #-z means "" is empty
			...
			#echo $(echo $info | awk '{print $2}')
			WN1n=$WN1n","$(echo $info | awk '{print $2}')
		fi
	fi

	info=$(egrep 'WN1p' $file_dir)
	if [[ -n "$(egrep 'Subject' $file_dir)" ]]; then #-n means ""is not empty
		WN1p=$WN1p", "
	else
		if [[ -z "$info" ]]; then #-z means "" is empty
			...
			#echo $(echo $info | awk '{print $2}')
			WN1p=$WN1p","$(echo $info | awk '{print $2}')
		fi
	fi

	info=$(egrep 'WN2n' $file_dir)
	if [[ -n "$(egrep 'Subject' $file_dir)" ]]; then #-n means ""is not empty
		WN2n=$WN2n", "
	else
		if [[ -z "$info" ]]; then #-z means "" is empty
			...
			#echo $(echo $info | awk '{print $2}')
			WN2n=$WN2n","$(echo $info | awk '{print $2}')
		fi
	fi

	info=$(egrep 'WN2p' $file_dir)
	if [[ -n "$(egrep 'Subject' $file_dir)" ]]; then #-n means ""is not empty
		WN2p=$WN2p", "
	else
		if [[ -z "$info" ]]; then #-z means "" is empty
			...
			#echo $(echo $info | awk '{print $2}')
			WN2p=$WN2p","$(echo $info | awk '{print $2}')
		fi
	fi
#O
	info=$(egrep 'WO1n' $file_dir)
	if [[ -n "$(egrep 'Subject' $file_dir)" ]]; then #-n means ""is not empty
		WO1n=$WO1n", "
	else
		if [[ -z "$info" ]]; then #-z means "" is empty
			...
			#echo $(echo $info | awk '{print $2}')
			WO1n=$WO1n","$(echo $info | awk '{print $2}')
		fi
	fi

	info=$(egrep 'WO1p' $file_dir)
	if [[ -n "$(egrep 'Subject' $file_dir)" ]]; then #-n means ""is not empty
		WO1p=$WO1p", "
	else
		if [[ -z "$info" ]]; then #-z means "" is empty
			...
			#echo $(echo $info | awk '{print $2}')
			WO1p=$WO1p","$(echo $info | awk '{print $2}')
		fi
	fi

	info=$(egrep 'WO2n' $file_dir)
	if [[ -n "$(egrep 'Subject' $file_dir)" ]]; then #-n means ""is not empty
		WO2n=$WO2n", "
	else
		if [[ -z "$info" ]]; then #-z means "" is empty
			...
			#echo $(echo $info | awk '{print $2}')
			WO2n=$WO2n","$(echo $info | awk '{print $2}')
		fi
	fi

	info=$(egrep 'WO2p' $file_dir)
	if [[ -n "$(egrep 'Subject' $file_dir)" ]]; then #-n means ""is not empty
		WO2p=$WO2p", "
	else
		if [[ -z "$info" ]]; then #-z means "" is empty
			...
			#echo $(echo $info | awk '{print $2}')
			WO2p=$WO2p","$(echo $info | awk '{print $2}')
		fi
	fi
#T
...
do
	...
	if [ $i == 47 ]; then name="WL1n" 
	fi
	if [ $i == 48 ]; then name="WL1p" 
	fi
	if [ $i == 49 ]; then name="WL2n" 
	fi
	if [ $i == 50 ]; then name="WL2p" 
	fi
	if [ $i == 51 ]; then name="WM1n" 
	fi
	if [ $i == 52 ]; then name="WM1p" 
	fi
	if [ $i == 53 ]; then name="WM2n" 
	fi
	if [ $i == 54 ]; then name="WM2p" 
	fi
	if [ $i == 55 ]; then name="WN1n" 
	fi
	if [ $i == 56 ]; then name="WN1p" 
	fi
	if [ $i == 57 ]; then name="WN2n" 
	fi
	if [ $i == 58 ]; then name="WN2p" 
	fi
	if [ $i == 59 ]; then name="WO1n" 
	fi
	if [ $i == 60 ]; then name="WO1p" 
	fi
	if [ $i == 61 ]; then name="WO2n" 
	fi
	if [ $i == 62 ]; then name="WO2p" 
	fi
	if [ $i == 63 ]; then name="WT1n" 
	fi
	if [ $i == 64 ]; then name="WT1p" 
	fi
	if [ $i == 65 ]; then name="WT2n" 
	fi
	if [ $i == 66 ]; then name="WT2p" 
	fi
